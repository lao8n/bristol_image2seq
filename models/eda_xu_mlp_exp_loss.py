...
Image2Seq Model = Encoder-Decoder with Attention (Xu et al) with MLP and
exponential loss
...
# Image2Seq Model                                                           #
#############################################################################
class EDAXUMLPEXPLOSS(keras.Model):
  def __init__(self,
    ...
    super(EDAXUMLPEXPLOSS, self).__init__()
  ...
  def call(self, inputs, val_mode=False, dropout=False):      
    ...
    logging.debug("MODEL EDA XU MLP EXP LOSS CALL - Step 0 - Process "
                  "inputs - batch_size {}".format(self.batch_size))
    logging.debug("MODEL EDA XU MLP EXP LOSS CALL - Step 0 - Process "
                  "inputs - input_image shape {}"
                  .format(K.int_shape(input_image)))
    logging.debug("MODEL EDA XU MLP EXP LOSS CALL - Step 0 - Process "
                  "inputs - input_tokens shape {}"
    ...
    # Logging, Debug & Assert
    logging.debug("MODEL EDA XU MLP EXP LOSS CALL - Step 1 - Reset decoder "
    ...
    # Logging, Debug & Assert
    logging.debug("MODEL EDA XU MLP EXP LOSS CALL - Step 2 - Image "
                  "encoding dense and activations - input_image_features "
                  "shape {}"
    ...
    # Logging, Debug & Assert 
    logging.debug("MODEL EDA XU MLP EXP LOSS CALL - Step 3 - Token "
                  "embeddings - target_token_embeddings shape {}"
    ...
    # Logging, Debug, & Assert
    logging.debug("MODEL EDA XU MLP EXP LOSS CALL - Step 4 - Decoder "
                  "inputs - decoder_teaching_forcing_inputs shape {}"
                  .format(K.int_shape(
                    decoder_token_input)))
    tf.compat.v1.debugging.assert_equal(
    ...
    for i in range(1, batch_token_seq_len):
      ...
      # Logging, Debug & Assert 
      logging.debug("MODEL EDA XU MLP EXP LOSS CALL - Step 5.1 - "
                    "Attention - Context vector shape {}"
      ...
      # Logging, Debug & Assert 
      logging.debug("MODEL EDA XU MLP EXP LOSS CALL - Step 5.2 - Expand "
                    "context vector - context_vector_expanded shape {}"
      ...
      # Logging, Debug & Assert  
      logging.debug("MODEL EDA XU MLP EXP LOSS CALL - Step 5.2 - Concat "
                    "context vector and token embedding - "
                    "lstm_input shape {}"
      ...
      # Logging, Debug & Assert
      logging.debug("MODEL EDA XU MLP EXP LOSS CALL - Step 5.3 - LSTM "
                    "output - lstm_output shape {}"
                    .format(K.int_shape(lstm_output)))
      logging.debug("MODEL EDA XU MLP EXP LOSS CALL - Step 5.3 - LSTM "
                    "output - decoder_hidden_state shape {}"
                    .format(K.int_shape(decoder_hidden_state)))
      logging.debug("MODEL EDA XU MLP EXP LOSS CALL - Step 5.3 - LSTM "
                    "output - decoder_cell_state shape {}"
      ...
      # Logging. Debug & Assert
      logging.debug("MODEL EDA XU MLP EXP LOSS CALL - Step 5.4 - MLP "
                    "output - single_token_prediction shape {}"
      ...
      #___________________|_____________________|___________________________#
      batch_loss += (-math.log(i / int(batch_token_seq_len)) + 1) * \
                    masked_ce_loss_fn(target=input_tokens[:, i],
                                      prediction=single_token_prediction,
                                      batch_size=self.batch_size,
                                      token_vocab_size=self.token_vocab_size)
  
      # Logging, Debug & Assert
      logging.debug("MODEL EDA XU MLP EXP LOSS CALL - Step 5.5 - "
      ...
      # Logging, Debug & Assert
      logging.debug("MODEL EDA XU MLP EXP LOSS CALL - Step 5.6 - Update "
                    "decoder input - decoder_token_input shape {}"
    ...
    if val_mode:
      ...
      logging.debug("MODEL EDA XU MLP EXP LOSS CALL - Step 6 - Stack "
                    "predictions shape {}"
