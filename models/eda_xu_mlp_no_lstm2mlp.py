...
# Image2Seq Model                                                           #
#############################################################################
class EDAXUMLPNOLSTM2MLP(keras.Model):
  def __init__(self,
    ...
    super(EDAXUMLPNOLSTM2MLP, self).__init__()
  ...
  def call(self, inputs, val_mode=False, dropout=False):      
    ...
    # Logging, Debug & Assert
    logging.debug("MODEL EDA XU MLP CALL - Step 2 - Image encoding dense"
    ...
    for i in range(1, batch_token_seq_len):
      ...
      # Logging, Debug & Assert  
      logging.debug("MODEL EDA XU MLP CALL - Step 5.2 - Concat context"
      ...
      single_token_prediction = self.model4_mlp([context_vector_expanded],
                                                dropout=dropout)
  ...
  def get_attention_predictions(self):
    return tf.stack(self.attention_predictions, axis=1)
