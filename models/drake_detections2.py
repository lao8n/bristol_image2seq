...
Image2Seq Model = Encoder-Decoder with Attention DRAKE DETECTIONS 2enated input
...
# Image2Seq Model                                                           #
#############################################################################
class DRAKEDETECTIONS2(keras.Model):
  def __init__(self,
  ...
  def call(self, inputs, val_mode=False, dropout=False):      
    ...
    if val_mode:
      logging.debug("MODEL DRAKE DETECTIONS 2 CALL - Train mode")
    else:
      logging.debug("MODEL DRAKE DETECTIONS 2 CALL - Validation mode")
    ...
    logging.debug("MODEL DRAKE DETECTIONS 2 CALL - Step 0 - Process inputs - "
    ...
    # Logging, Debug & Assert
    logging.debug("MODEL DRAKE DETECTIONS 2 CALL - Step 1 - Image encoding dense"
    ...
    # STEP 2: Set Decoder Hidden State ######################################
    # Assign              | Initial decoder     | batch_size=None x         #
    #                     | hidden state        | decoder_hidden_dim=       #
    #                     |                     | decoder_hidden_dim        #
    #_____________________|_____________________|___________________________#
    decoder_hidden_state = \
      self.model5_detections2hidden([input_image_features],
                                    dropout=dropout)    
    # Logging, Debug & Assert
    logging.debug("MODEL DRAKE DETECTIONS 2 CALL - Step 2 - Initial hidden "
    ...
    # Logging, Debug & Assert 
    logging.debug("MODEL DRAKE DETECTIONS 2 CALL - Step 3 - Token embeddings - "
    ...
    # Logging, Debug, & Assert
    logging.debug("MODEL DRAKE DETECTIONS 2 CALL - Step 4 - Decoder inputs - "
    ...
    for i in range(1, batch_token_seq_len):
      ...
      context_vector, attention_weights = \
        self.model3_attention([retinanet_detections_token_embedded, 
                               decoder_hidden_state],
                              dropout=dropout)
      
      # Logging, Debug & Assert 
      logging.debug("MODEL DRAKE DETECTIONS 2 CALL - Step 5.1 - Attention - "
      ...
      # Logging, Debug & Assert 
      logging.debug("MODEL DRAKE DETECTIONS 2 CALL - Step 5.2 - Expand context "
      ...
      # Logging, Debug & Assert  
      logging.debug("MODEL DRAKE DETECTIONS 2 CALL - Step 5.2 - Concat context"
      ...
      # Logging, Debug & Assert
      logging.debug("MODEL DRAKE DETECTIONS 2 CALL - Step 5.3 - LSTM output - "
                    "lstm_output shape {}"
      ...
      # Logging. Debug & Assert
      logging.debug("MODEL DRAKE DETECTIONS 2 CALL - Step 5.4 - MLP output - "
      ...
      # Logging, Debug & Assert
      logging.debug("MODEL DRAKE DETECTIONS 2 CALL - Step 5.5 - "
      ...
      # Logging, Debug & Assert
      logging.debug("MODEL DRAKE DETECTIONS 2 CALL - Step 5.6 - Update decoder "
    ...
    if val_mode:
      ...
      logging.debug("MODEL DRAKE DETECTIONS 2 CALL - Step 6 - Stack predictions "
                    "shape {}"
