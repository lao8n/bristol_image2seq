...
Image2Seq Model = Encoder-Decoder with Attention (Xu et al) - no Att->MLP
...
class EDAXU(keras.Model):
  ...
  def call(self, inputs, val_mode=False, dropout=False):      
    ...
    if val_mode:
      logging.debug("MODEL EDA XU CALL - Train mode")
    else:
      logging.debug("MODEL EDA XU CALL - Validation mode")
    ...
    logging.debug("MODEL EDA XU CALL - Step 0 - Process inputs - "
    ...
    # Logging, Debug & Assert
    logging.debug("MODEL EDA XU CALL - Step 1 - Reset decoder hidden "
    ...
    # Logging, Debug & Assert
    logging.debug("MODEL EDA XU CALL - Step 2 - Image encoding dense"
    ...
    # Logging, Debug & Assert 
    logging.debug("MODEL EDA XU CALL - Step 3 - Token embeddings - "
    ...
    # Logging, Debug, & Assert
    logging.debug("MODEL EDA XU CALL - Step 4 - Decoder inputs - "
    ...
    for i in range(1, batch_token_seq_len):
      ...
      # Logging, Debug & Assert 
      logging.debug("MODEL EDA XU CALL - Step 5.1 - Attention - "
      ...
      # Logging, Debug & Assert 
      logging.debug("MODEL EDA XU CALL - Step 5.2 - Expand context "
      ...
      # Logging, Debug & Assert  
      logging.debug("MODEL EDA XU CALL - Step 5.2 - Concat context"
      ...
      # Logging, Debug & Assert
      logging.debug("MODEL EDA XU CALL - Step 5.3 - LSTM output - "
                    "lstm_output shape {}"
      ...
      logging.debug("MODEL EDA XU CALL - Step 5.3 - LSTM output - "
                    "decoder_hidden_state \n{}"
                    .format(decoder_hidden_state))
      logging.debug("MODEL EDA XU CALL - Step 5.3 - LSTM output - "
                    "decoder_cell_state \n{}"
                    .format(decoder_cell_state))
      tf.compat.v1.debugging.assert_equal(
      ...
      # Logging. Debug & Assert
      logging.debug("MODEL EDA XU CALL - Step 5.4 - MLP output - "
      ...
      # Logging, Debug & Assert
      logging.debug("MODEL EDA XU CALL - Step 5.5 - "
      ...
      # Logging, Debug & Assert
      logging.debug("MODEL EDA XU CALL - Step 5.6 - Update decoder "
